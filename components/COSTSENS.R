...
ui = tags$span(showOutput("demo4DASHBOARD.uuid", "highcharts"),
...
output$plotDASHBOARD.uuid <- renderPlot({ par(mar=c(0,0,0,0)); plot(1, type="n", axes=F, xlab="", ylab="") })
    ...
    output$demo4DASHBOARD.uuid <- renderChart({      
      ...
      if (!is.null(thewater) & !is.null(theuclf) & !is.null(theuclf2) & !is.null(thecountry)   ) {
        r1 = demo4(thewater,theuclf,theuclf2,thecountry, thedom="demo4DASHBOARD.uuid","","All",
